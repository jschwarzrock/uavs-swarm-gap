__includes [ "bases/base.nls" "bases/input.nls" "bases/create_breeds.nls"  "bases/report.nls"  "bases/perform_tasks.nls" "bases/allocation.nls"
             "bases-swarm-gap/base-swarm-gap-metodo-inicial.nls"
             "base-loop.nls"]

to inicio-local
  set nome-algoritmo "Swarm-GAP (com LOOP alocação e Ordenacao)" ;; Sorting and Allocation Loop (SAL)
end
...
to choose_task_ordenada_por_t[i]
  ...
  foreach sort-by [item 2 ?1 > item 2 ?2] tendencias[ ;; Percorre de forma ordenada pela tendência
    ...
    if (roulette < T) and (r > consumo) [
        ...
        ;set alocadas (alocadas + 1)
        ;if alocadas = 1 [
          ;stop
        ;]

         ;; aloca 2 tarefa -- para isso funcionar, vai precisar computar a distância novamente....
    ]
  ]
end
